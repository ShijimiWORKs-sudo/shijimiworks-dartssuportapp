...
| F-002 | プロフィール再編集                            | `app/settings.tsx`                                         | rating、machine、悩み、テーマを編集        | KEEP            | Support用途の個人設定                   | `saveProfileAndDisplaySettings` | OWNER accountId連携の継続QA            |
...
| F-014 | 練習記録一覧/詳細/編集/削除                   | `app/records*.tsx`                                         | CRUD実装済み、既存record互換あり           | KEEP            | 練習履歴管理の主機能                    | AppState.records                | accountId未設定recordのQA継続          |
...
| F-031 | 品質チェック                                  | `tests/*`, `utils/validateDataIntegrity.ts`                | 98件規模のロジックテストと整合性           | KEEP            | 追加機能の破損防止                      | Node test                       | Account回帰テスト維持                  |
...
| F-047 | Account Export/Import UI                      | `app/account/export.tsx`, `app/account/import.tsx`         | 共通JSON Export/Import previewを実装       | KEEP            | ローカルBackup/RestoreのMVP導線         | Account, records                | 復元ガードと文言をP1で強化             |
| F-048 | PC Web詳細分析domain                          | 一部utilあり                                               | UI未実装                                   | MOVE_LATER      | 将来Web版に流用                         | analysis utils                  | P3で分離                               |
| F-049 | UserProfile                                   | `types/index.ts`                                           | rating/level/machine/problems/accountId    | SHARED_CONTRACT | OWNER profileと共通account_idの接続対象 | profile                         | DartsApp側contractと命名維持           |
| F-050 | PracticeRecord                                | `types/index.ts`                                           | 簡易記録、photoScore、optional accountId   | SHARED_CONTRACT | DartsApp結果分析にも利用                | records                         | game_session_idは将来検討              |
...
| F-062 | schemaVersion/migration                       | `utils/appStateMigration.ts`                               | schemaVersion 10                           | SHARED_CONTRACT | 既存データ互換とAccount追加の境界       | AsyncStorage                    | 冪等migrationを維持                    |
| F-063 | Account contract docs                         | `docs/specs/Darts_Common_Account_Data_Contract_v1.0.md`    | 仕様書と実装がmainへ反映済み               | SHARED_CONTRACT | DartsApp共通ID契約                      | Account implementation          | DartsApp側実装時に照合                 |
| F-064 | Export/Import contract                        | `features/account/application/commonContract*.ts`          | 共通JSON Export/Import MVP実装済み         | SHARED_CONTRACT | Backup/Restore/共通JSON対象             | Account, records                | conflict handlingの拡張                |
| F-065 | CommonEvent/Outbox                            | `CommonOutboxItem`, `commonOutbox`                         | 将来同期用Outboxをローカル保持             | SHARED_CONTRACT | 将来同期契約                            | AppState                        | Supabase等は未実装のまま維持           |
| F-066 | Account画面群                                 | `app/account/*`                                            | 一覧/登録/プロフィール/セキュリティ等実装  | KEEP            | Account管理はSupport側設定機能          | Account contract                | iPhone実機QA継続                       |
| F-067 | PIN/SecureStore                               | `expo-secure-store`, `pinService.ts`                       | 4〜8桁PINをSecureStoreへ保存               | KEEP            | PIN秘密情報をAppState/JSONへ保存しない  | SecureStore                     | Export除外テスト維持                   |
| F-068 | activeAccountId/accountId                     | `types/index.ts`, `AppStateContext.tsx`                    | AppState/profile/新規recordへ反映済み      | SHARED_CONTRACT | 共通契約の主キー接続                    | migration                       | 既存record互換を維持                   |
| F-069 | JSON Export/Import UI                         | `app/account/export.tsx`, `app/account/import.tsx`         | Export、Import検証、previewを実装          | KEEP            | ローカル復元導線として必要              | common contract                 | 不正JSON/別account preview QA          |
...
| F-073 | Account docs vs current docs                  | `docs/ARCHITECTURE.md`, `QA_CHECKLIST.md`                  | schemaVersion 10 / Account実装記述         | KEEP            | Account実装後の運用文書                 | docs                            | 今後の同期仕様追加時に更新             |
