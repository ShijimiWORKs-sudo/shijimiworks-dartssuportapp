...
- 今日の練習: メニュー追加、実施順、タイマー、一時停止/再開、完了入力、練習記録化
...
今日の練習MVPは練習支援用の実施管理です。正式な01ゲームエンジン、STANDARD CRICKET対戦、MATCH、BUST、プレイヤー交代処理は含みません。

今日の練習の流れ:

1. `/practice/today` で今日の練習一覧を開きます。
2. `/practice/today/select` でおすすめまたは練習メニューDBから追加します。
3. `/practice/session/[id]` でタイマーを開始、一時停止、再開します。
4. `/practice/session/[id]/complete` で実施時間、完了R/セット、スコア、Bull数、調子、メモを入力します。
5. 保存すると通常の練習記録として `/analysis` に反映されます。
