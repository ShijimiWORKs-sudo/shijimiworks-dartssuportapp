...
- 練習記録: 入力、一覧、詳細、編集、削除
- 写真スコア記録: 画像解析候補、半自動候補、手動タップ、ドラッグ微調整で3本分のスコアとグルーピング傾向を保存
- 分析: 期間別集計、ゲーム別集計、簡易グラフ、改善コメント
- フォーム相談: カテゴリ別の固定アドバイス表示
- フォーム写真3枚相談: 正面、横、リリース後写真と自己チェック、直近写真スコアから固定ロジック助言
...
3. 画像候補、半自動候補、手動タップから3本の位置を選びます。
4. 必要に応じて選択点をドラッグ調整します。
5. 結果画面でスコア、判定元、グルーピング分析、アドバイスを確認して保存します。
